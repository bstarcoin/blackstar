...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("blackstar"))
        return false;
...
{
    // Convert blackstar:// to blackstar:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("blackstar://"))
    {
        uri.replace(0, 12, "blackstar:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Blackstar.lnk";
}
...
{
    return GetAutostartDir() / "blackstar.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Blackstar-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  blackstar-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Blackstar-Qt"));
